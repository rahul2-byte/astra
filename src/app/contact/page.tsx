...
export const metadata: Metadata = {
  title: "Contact",
  description: "Contact Rahul Singh for Machine Learning Engineer, Applied AI Engineer, LLM/RAG Engineer, and Data Scientist opportunities.",
  openGraph: {
    title: "Contact | Rahul Singh",
    description: "Direct contact details and resume access for Rahul Singh, Machine Learning Engineer.",
  },
};
    ...
    <main className="section-shell py-16 md:py-24">
      <section className="technical-grid relative mx-auto max-w-5xl overflow-hidden border-y soft-divider py-14 text-center">
        ...
        <div className="relative">
          ...
          <h1 className="font-display mt-5 text-5xl font-semibold leading-tight tracking-[-0.055em] md:text-6xl">Contact Rahul Singh</h1>
          <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-[var(--muted)]">Open to Machine Learning Engineer, Applied AI Engineer, LLM/RAG Engineer, and Data Scientist opportunities.</p>
        </div>
      </section>
        ...
        {contactMethods.map(({ title, href, label, Icon, external }, index) => (
          ...
          >
            <div className="flex items-start justify-between">
              <span className="grid h-12 w-12 place-items-center bg-[var(--primary)]"><Icon className="h-5 w-5" strokeWidth={1.7} aria-hidden /></span>
              <span className="font-technical text-xs text-[var(--muted)]">0{index + 1}</span>
            </div>
            <div className="mt-8">
              <span className="flex items-center gap-2 text-xl font-semibold tracking-[-0.03em]">{title}{external ? <ArrowUpRight className="link-arrow h-4 w-4" aria-hidden /> : null}</span>
      ...
      <div className="mt-12 flex justify-center">
        <a className="btn-primary" download="Rahul-Singh-ML-Engineer-Resume.pdf" href={site.resume}>
          <Download className="h-4 w-4" aria-hidden />Download Technical Resume
        </a>
      </div>
    </main>
  );
